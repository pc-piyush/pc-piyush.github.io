local function parse_yaml(content)
    ...
    for line in content:gmatch("[^\r\n]+") do
        ...
        elseif in_description and indent_level > description_indent then
            -- Continuation line of a multi-line description
            item.description = item.description .. " " .. line_trimmed
        ...
        elseif indent_level > 0 and line_trimmed:match("^date:%s*(.+)") then
            ...
            in_description = false
        elseif indent_level > 0 and line_trimmed:match("^description:%s*[|>][%-%+]?%s*$") then
            -- Block scalar description (`description: |` or `description: >`),
            -- content follows on subsequent, more-indented lines
            item.description = ""
            in_description = true
            description_indent = indent_level
